//
const SET_ADDRESS_DIRECTORY_LIST = 'set_address_directory_list';
const UPDATE_ADDRESS_DIRECTORY = 'update_address_directory';
const ADD_ADDRESS_DIRECTORY = 'add_address_directory';
//
const SET_PROFILE = 'set_profile';
const UPDATE_PROFILE = 'update_profile';
//
const SET_CART = 'set_cart';
const DELETE_CART = 'delete_cart';
const ADD_CART = 'add_cart';
const RESET_CART = 'reset_cart';
//
const SET_CUSTOMERS = 'set_customers';
const ADD_CUSTOMER = 'add_customer';
//
const RESET_FILTER = 'reset_filter';
const PRODUCTS_GENDER = 'products_gender';
const PRODUCTS_COLOR = 'products_color';
const PRODUCTS_SIZE = 'products_size';
const PRODUCTS_PRICE = 'products_price';
const DELETE_PRODUCTS_GENDER = 'delete_products_gender';
const DELETE_PRODUCTS_COLOR = 'delete_products_color';
const DELETE_PRODUCTS_SIZE = 'delete_products_size';
const DELETE_PRODUCTS_PRICE = 'delete_products_price';

export {
  SET_ADDRESS_DIRECTORY_LIST,
  UPDATE_ADDRESS_DIRECTORY,
  ADD_ADDRESS_DIRECTORY,
  //
  SET_PROFILE,
  UPDATE_PROFILE,
  //
  SET_CART,
  DELETE_CART,
  ADD_CART,
  RESET_CART,
  //
  SET_CUSTOMERS,
  ADD_CUSTOMER,
  //
  RESET_FILTER,
  PRODUCTS_GENDER,
  PRODUCTS_COLOR,
  PRODUCTS_SIZE,
  PRODUCTS_PRICE,
  DELETE_PRODUCTS_GENDER,
  DELETE_PRODUCTS_COLOR,
  DELETE_PRODUCTS_SIZE,
  DELETE_PRODUCTS_PRICE,
};
